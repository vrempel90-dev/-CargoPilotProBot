...
def telegram_menu_url() -> str:
    """Ссылка из веб-трекера обратно в главное меню Telegram-бота."""
    return f"https://t.me/{BOT_USERNAME}?start=menu"


def track_button(code: str) -> InlineKeyboardMarkup:
    """Кнопки под статусом груза для клиента.

    Человек не должен думать, куда нажимать дальше: открыть трекер,
    написать менеджеру или вернуться в меню.
    """
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🔎 Открыть трекер груза", url=track_url(code))],
            [InlineKeyboardButton(text="🛠️ Написать менеджеру", callback_data="support_new")],
            [InlineKeyboardButton(text="↩️ Вернуться в меню", callback_data="main_menu_back")],
        ]
    )
...
def track_pay_button(code: str, debt: float = 0) -> InlineKeyboardMarkup:
    buttons = [[InlineKeyboardButton(text="🔎 Открыть трекер груза", url=track_url(code))]]
    if DEMO_PAYMENT_ENABLED and debt > 0:
        buttons.append([InlineKeyboardButton(text="💳 Оплатить доставку", url=payment_page_url(code))])
    buttons.append([InlineKeyboardButton(text="🛠️ Написать менеджеру", callback_data="support_new")])
    buttons.append([InlineKeyboardButton(text="↩️ Вернуться в меню", callback_data="main_menu_back")])
...
def client_keyboard() -> ReplyKeyboardMarkup:
    """Главное меню клиента: только действия, которые реально нужны клиенту.

    На первом экране нет внутренних слов вроде CRM, Promise OS, Excel, роли.
    Клиент видит простой сервис доставки: оформить, отследить, оплатить, написать менеджеру.
    """
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📦 Оформить доставку"), KeyboardButton(text="🔎 Отследить груз")],
            [KeyboardButton(text="💳 Оплата"), KeyboardButton(text="📋 Мои заказы")],
            [KeyboardButton(text="🧮 Расчёт"), KeyboardButton(text="🛠️ Менеджер")],
            [KeyboardButton(text="☰ Ещё")],
        ],
        resize_keyboard=True,
        input_field_placeholder="Напишите номер груза CG... или выберите действие",
    )


def client_more_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🛒 Выкуп товара"), KeyboardButton(text="🏢 Оптовая доставка")],
            [KeyboardButton(text="⚠️ Сообщить о проблеме"), KeyboardButton(text="❓ Помощь / FAQ")],
            [KeyboardButton(text="🤝 Партнёрская ссылка"), KeyboardButton(text="👤 Личный кабинет")],
...
def admin_keyboard() -> ReplyKeyboardMarkup:
    """Главное меню админа: как рабочий стол CRM, без перегруза."""
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📥 Заявки"), KeyboardButton(text="🔎 Найти груз")],
            [KeyboardButton(text="📦 Грузы"), KeyboardButton(text="🔁 Статус")],
            [KeyboardButton(text="💳 Оплаты"), KeyboardButton(text="🚨 Проблемы")],
            [KeyboardButton(text="🤖 Автостатусы"), KeyboardButton(text="📊 Отчёт")],
            [KeyboardButton(text="🏭 Склад"), KeyboardButton(text="⚙️ Ещё")],
        ],
        resize_keyboard=True,
        input_field_placeholder="Введите CG... или выберите действие",
    )


def admin_more_keyboard() -> ReplyKeyboardMarkup:
    """Дополнительное меню админа для управления и настроек."""
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🧭 Контроль обещаний"), KeyboardButton(text="🔐 Безопасный срок")],
            [KeyboardButton(text="📦 Партии"), KeyboardButton(text="💬 Жалобы")],
            [KeyboardButton(text="👥 Клиенты"), KeyboardButton(text="👥 Доступы")],
            [KeyboardButton(text="📢 Рассылка"), KeyboardButton(text="📤 Excel экспорт")],
            [KeyboardButton(text="📥 Обновить из Excel"), KeyboardButton(text="📄 PDF квитанция")],
            [KeyboardButton(text="⚙️ Тарифы"), KeyboardButton(text="⚙️ Статусы")],
            [KeyboardButton(text="🚚 Курьеры"), KeyboardButton(text="👤 Клиентское меню")],
            [KeyboardButton(text="↩️ Главное меню")],
        ],
        resize_keyboard=True,
        input_field_placeholder="Настройки, экспорт и дополнительные инструменты",
    )
...
def warehouse_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="➕ Принять груз"), KeyboardButton(text="⚖️ Вес")],
            [KeyboardButton(text="📸 Фото"), KeyboardButton(text="🔁 Статус")],
            [KeyboardButton(text="📦 На складе"), KeyboardButton(text="🔎 Найти груз")],
            [KeyboardButton(text="🚚 Курьеры"), KeyboardButton(text="↩️ Главное меню")],
        ],
        resize_keyboard=True,
...
def courier_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🚚 Мои доставки"), KeyboardButton(text="✅ Доставлено")],
            [KeyboardButton(text="🔎 Найти груз"), KeyboardButton(text="↩️ Главное меню")],
        ],
        resize_keyboard=True,
        input_field_placeholder="Курьер: выберите действие или введите CG...",
    )
...
def client_menu_text(name: str = "") -> str:
    ...
    return (
        ...
        f"Здравствуйте{greeting}! Это ваш помощник по доставке.\n\n"
        "Здесь можно быстро:\n"
        "• оформить доставку;\n"
        "• отследить груз по номеру CG;\n"
        "• открыть оплату;\n"
        "• написать менеджеру.\n\n"
        "Просто нажмите нужную кнопку. Если у вас уже есть номер груза — отправьте его в чат."
    )


def admin_menu_text() -> str:
    return (
        f"<b>👑 Рабочий стол {safe(COMPANY_NAME)}</b>\n\n"
        "Всё главное на первом экране: заявки, грузы, оплаты, статусы и отчёт.\n\n"
        "<b>Рабочий сценарий:</b>\n"
        "1) принять заявку;\n"
        "2) подтвердить оплату;\n"
        "3) поставить безопасный срок;\n"
        "4) включить автостатусы;\n"
        "5) смотреть проблемы и отчёт.\n\n"
        "Настройки, Excel, роли, партии и рассылки находятся в <b>⚙️ Ещё</b>."
    )


def admin_more_text() -> str:
    return (
        f"<b>⚙️ Управление {safe(COMPANY_NAME)}</b>\n\n"
        "Здесь то, что не нужно каждую минуту: настройки, Excel, доступы, партии, рассылки и контроль обещаний.\n"
        "Для обычной работы возвращайтесь в <b>↩️ Главное меню</b>."
    )


def client_more_text() -> str:
    return (
        "<b>☰ Ещё</b>\n\n"
        "Дополнительные разделы: выкуп товара, оптовая доставка, проблема по грузу, FAQ, партнёрская ссылка и личный кабинет."
    )
...
def warehouse_menu_text() -> str:
    return (
        ...
        "Меню сделано для работы с телефона: принять груз, указать вес, добавить фото и обновить статус.\n\n"
...
def courier_menu_text() -> str:
    return (
        "<b>🚚 Курьер</b>\n\n"
        "Здесь только рабочие действия: мои доставки, найти груз, сменить статус и отметить доставку."
    )
...
def format_client_order(order: aiosqlite.Row) -> str:
    """Публичная карточка груза без внутренних админских полей."""
    debt = max(float(order["price"] or 0) - float(order["paid_amount"] or 0), 0)
    payment_line = "оплачено" if debt <= 0 else f"к оплате {debt:g} {safe(CURRENCY)}"
    trust = trustflow_info(order) if TRUSTFLOW_ENABLED else None
    next_line = ""
    if trust:
        next_due = f" до {safe(trust['next_due_text'])}" if trust.get("next_due_text") else ""
        next_line = f"\nСледующий этап: {safe(trust['next_stage'])}{next_due}"
    return (
        f"<b>📦 Груз {safe(order['tracking_code'])}</b>\n\n"
        f"Маршрут: {safe(order['from_country'])} → {safe(order['to_city'])}\n"
        f"Товар: {safe(order['cargo_type'])}\n"
        f"Вес: {safe(order['weight'])} кг\n\n"
        f"Статус: <b>{safe(order['status'])}</b>\n"
        f"Оплата: {safe(payment_line)}"
        f"{next_line}\n\n"
        "Откройте трекер, чтобы посмотреть историю, срок и детали по грузу."
    )


def format_client_short_order(order: aiosqlite.Row) -> str:
    return (
        f"• <b>{safe(order['tracking_code'])}</b> — {safe(order['status'])}\n"
        f"  {safe(order['from_country'])} → {safe(order['to_city'])}, {safe(order['weight'])} кг"
    )
...
def client_promise_created_text(order: aiosqlite.Row, estimate: Optional[float] = None, order_name: str = "Заявка") -> str:
    safe_window, start_rule, warning = promise_route_window(order)
    estimate_line = f"Примерная стоимость: <b>{estimate} {safe(CURRENCY)}</b>\n" if estimate is not None else ""
    return (
        f"✅ <b>{safe(order_name)} принята</b>\n\n"
        f"Номер груза: <b>{safe(order['tracking_code'])}</b>\n"
        f"{estimate_line}"
        f"Ориентир по сроку: <b>{safe(safe_window)}</b>\n"
        f"Срок считается: {safe(start_rule)}\n\n"
        "<b>Что дальше:</b>\n"
        "1) откройте оплату;\n"
        "2) менеджер подтвердит заявку;\n"
        "3) после обработки груза статус будет обновляться автоматически.\n\n"
        "Статус всегда можно проверить по кнопке ниже."
    )
...
async def cmd_reply_ticket(message: Message, bot: Bot):
    ...
    await notify_user(
        bot,
        ticket["user_id"],
        f"🛠️ Ответ менеджера по обращению №{ticket['id']}:\n\n{safe(parts[2])}\n\nЕсли вопрос не решён, нажмите «🛠️ Менеджер» → «📋 Мои обращения» и ответьте в этот тикет.",
    )
...
async def show_main_menu(message: Message, state: FSMContext):
    ...
    user_id = message.from_user.id if message.from_user else 0
    name = message.from_user.first_name if message.from_user else ""
    text, keyboard = await menu_for_user(user_id, name)
    await message.answer(text, reply_markup=keyboard)


@router.callback_query(F.data == "main_menu_back")
async def cb_main_menu_back(call: CallbackQuery, state: FSMContext):
    await state.clear()
    user_id = call.from_user.id if call.from_user else 0
    name = call.from_user.first_name if call.from_user else ""
    text, keyboard = await menu_for_user(user_id, name)
...
async def show_client_more_menu(message: Message, state: FSMContext):
    await state.clear()
    await message.answer(client_more_text(), reply_markup=client_more_keyboard())


@router.message(F.text == "⚙️ Ещё")
async def show_admin_more_menu(message: Message, state: FSMContext):
    if not message.from_user or not is_admin(message.from_user.id):
        return
    await state.clear()
    await message.answer(admin_more_text(), reply_markup=admin_more_keyboard())
...
@router.message(F.text == "🔎 Отследить груз")
...
async def track_start(message: Message, state: FSMContext):
    ...
    await message.answer("Напишите номер груза. Например: <code>CG26050300001</code>", reply_markup=client_keyboard())
...
async def track_finish(message: Message, state: FSMContext):
    ...
    await state.clear()
    if not order:
        await message.answer("Груз не найден. Проверьте номер CG или нажмите «🛠️ Менеджер», чтобы написать нам.", reply_markup=client_keyboard())
        return
    debt = max(float(order["price"] or 0) - float(order["paid_amount"] or 0), 0)
    await message.answer(format_client_order(order), reply_markup=track_pay_button(order['tracking_code'], debt))
...
async def payment_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Напишите номер груза для оплаты. Например: <code>CG26050300001</code>",
        reply_markup=client_keyboard(),
    )
...
async def payment_finish(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔎 Открыть трекер груза", url=track_url(order["tracking_code"]))],
            [InlineKeyboardButton(text="↩️ Вернуться в меню", callback_data="main_menu_back")],
        ]),
    )
...
async def my_orders(message: Message):
    ...
    if not orders:
        ...
        return
    text = "<b>📋 Ваши последние заказы</b>\n\n" + "\n\n".join(format_client_short_order(o) for o in orders)
...
async def support_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "<b>🛠️ Менеджер</b>\n\n"
        "Здесь можно написать менеджеру по грузу, оплате, фото, повреждению или работе бота.\n\n"
        "Выберите действие:",
...
async def support_finish(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.answer(
        ...
        "Чтобы посмотреть свои обращения или дописать сообщение, нажмите «🛠️ Менеджер» → «📋 Мои обращения».",
        reply_markup=client_keyboard(),
    )
    await notify_admins(
        bot,
...
async def faq(message: Message):
    await message.answer(
        "<b>❓ Помощь</b>\n\n"
        "<b>Как узнать статус?</b> Нажмите «🔎 Отследить груз» или просто отправьте номер CG в чат.\n\n"
        "<b>Цена финальная?</b> Расчёт предварительный. Финальная цена зависит от фактического веса, объёма и категории товара.\n\n"
        "<b>Какие документы нужны?</b> Обычно нужны описание товара, количество, стоимость, инвойс/ссылка/упаковочный лист, если есть.\n\n"
        "<b>Что делать при проблеме?</b> Нажмите «⚠️ Сообщить о проблеме» или «🛠️ Менеджер».",
        reply_markup=client_keyboard(),
    )

@router.message(F.text == "🤝 Партнёрская ссылка")
...
async def client_cabinet(message: Message):
    ...
    await message.answer(
        ...
        f"Долг: {debt} {safe(CURRENCY)}\n\n"
        "Для статуса нажмите «🔎 Отследить груз» или просто отправьте номер CG в чат.",
        reply_markup=client_keyboard(),
    )
...
@router.message(F.text == "✅ Доставлено")
...
async def admin_find_start(message: Message, state: FSMContext):
    if not message.from_user:
        return
    user_id = message.from_user.id
    allowed = is_admin(user_id) or is_warehouse(user_id) or is_courier_static(user_id) or await has_role(user_id, "warehouse", "courier")
    if not allowed:
        return
    await state.clear()
    await state.set_state(TrackForm.code)
...
@router.message(F.text == "🔐 Безопасный срок")
...
@router.message(F.text == "🧭 Контроль обещаний")
...
async def cb_support_template(call: CallbackQuery, bot: Bot):
    ...
    if not ticket:
        ...
        return
    await notify_user(bot, ticket["user_id"], f"🛠️ Ответ менеджера по обращению №{ticket_id}:\n\n{safe(text)}")
...
@router.message(F.text == "📦 На складе")
...
@router.message(F.text.regexp(r"(?i)\bCG\d{6}\d{4,}\b"))
async def quick_track_by_code(message: Message, state: FSMContext):
    """Клиент может просто отправить номер CG без поиска нужной кнопки."""
    if await state.get_state():
        return
    code = parse_tracking_code(message.text or "")
    if not code:
        return
    order = await get_order_by_code(code)
    if not order:
        await message.answer("Груз не найден. Проверьте номер CG или нажмите «🛠️ Менеджер».", reply_markup=client_keyboard())
        return
    debt = max(float(order["price"] or 0) - float(order["paid_amount"] or 0), 0)
    await message.answer(format_client_order(order), reply_markup=track_pay_button(order["tracking_code"], debt))
...
async def cb_promise_card(call: CallbackQuery):
    ...
    await call.message.answer(
        ...
        f"Трекер клиента: {safe(track_url(order['tracking_code']))}",
...
async def admin_support_reply_finish(message: Message, state: FSMContext, bot: Bot):
    ...
    await notify_user(
        bot,
        ticket["user_id"],
        f"🛠️ Ответ менеджера по обращению №{ticket_id}:\n\n{safe(message.text)}\n\nЕсли вопрос не решён, нажмите «🛠️ Менеджер» → «📋 Мои обращения» и ответьте в этот тикет.",
    )
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .telegram-menu {{ display:inline-block; padding:12px 14px; border-radius:14px; background:#eef2f7; color:#0f2744; font-weight:800; }}
...
<body>
  <div class="wrap">
    <div class="shell">
      <div class="hero">
        <h1>{company}</h1>
        <p>Статус груза, срок доставки и история обновлений</p>
      </div>
      <div class="card">
        <div class="top-actions"><a class="telegram-menu" href="{telegram_menu_url()}">↩️ Вернуться в Telegram</a></div>
        {content}
      </div>
      <div class="footer">Личный кабинет груза · статус, сроки и история</div>
    </div>
  </div>
</body>
</html>"""
...
def _track_form(code: str = "", error: str = "") -> str:
    ...
    return f"""
      <h2 style="margin:0 0 8px;">Проверить груз</h2>
      ...
      <div class="hint">Если номер не найден, вернитесь в Telegram и напишите менеджеру.</div>
    """


async def landing_page(_: web.Request) -> web.Response:
    return _web_response(_track_layout("Проверить груз", _track_form()))
